...
class _TeacherAttendanceScreenState extends State<TeacherAttendanceScreen> {
  ...
  Future<bool> _isWithinSchoolPremises(String? instituteId) async {
    ...
    try {
      ...
      if (instituteId != null && instituteId.isNotEmpty) {
        ...
        if (currentUser != null) {
          doc = await gpsCollection.doc(currentUser.uid).get();
          if (!doc.exists) {
            doc = await gpsCollection.doc('config').get();
          }
        } else {
          doc = await gpsCollection.doc('config').get();
        }
  ...
  // 📸 CAPTURE & UPLOAD FLOW
  Future<void> _captureAndMarkPresent(
    String studentId,
    String studentName,
    ...
    final instituteId =
        (studentData['instituteId'] ?? studentData['instituteName'] ?? '')
            .toString();
    bool isAllowed = await _isWithinSchoolPremises(
      instituteId.isEmpty ? null : instituteId,
    );
    if (!isAllowed) return;
    ...
    try {
      ...
      final instituteName =
          (studentData['instituteName'] ?? studentData['instituteId'] ?? 'unknown_institute')
              .toString();
      final batchName =
          (studentData['batchName'] ?? studentData['batch'] ?? studentData['year'] ?? 'general_batch')
              .toString();
      final subject = (studentData['subject'] ?? 'general_subject').toString();
      final lectureKey =
          (studentData['lecture'] ?? studentData['subject'] ?? 'manual').toString();
      ...
      String storedSelfiePath = "";
      int attempts = 0;
      while (attempts < 3) {
        try {
          storedSelfiePath = await BackblazeB2SecureService.uploadFile(
            objectPath: filePath,
      ...
      if (storedSelfiePath.isEmpty) throw "Upload failed.";
      ...
      await FirebaseFirestore.instance.collection('attendance').doc(docId).set({
        ...
        'markedBy': 'Teacher',
        'isManual': true,
        'verificationSelfie': storedSelfiePath,
        'photoPath': filePath,
  ...
  Future<void> _viewProofImage(String selfieRef, String name, String time) async {
    final url = await _resolveSelfieUrl(selfieRef);
    if (!mounted) return;
    if (url == null || url.isEmpty) {
      if (mounted) {
        _showErrorDialog("Image Error", "Could not open proof image.");
      }
      return;
    }
    showDialog(
  ...
  Widget _buildManualActions(
    ...
    return Row(
      ...
      children: [
        ...
        ElevatedButton.icon(
          onPressed: () => _captureAndMarkPresent(id, name, studentData),
